...
def plot_disjoint_communities_igraph(graph, 
                                     partition, 
                                     ax=None):
...
def graph_generation_or_importation(number_of_nodes=None, 
                                    degree_mean=1, 
                                    execute_graph=True, 
                                    file_name=None, 
                                    ax = None):
    ...
    if execute_graph:
        if number_of_nodes is None:
            raise ValueError("number_of_nodes must be provided when execute_graph is True")
...
def get_params(g, 
    ...
    for node in nodes:
        ...
        if len(parents) == 0:
            ...
            var = random.uniform(1, 3)         
            intercept[node] = np.random.choice([0,1])
...
def correlation_fault(non_root_coefs, 
                      g, 
                      k=0, 
                      affect_edge=None):
...
def add_measurement_noise(data, 
                          nodes, 
                          snr_db,
                          sensor_fault=0,
                          faulty_nodes=None):
    ...
    for node, values in data.items():
        ...
        if node in faulty_nodes and sensor_fault > 0:
            current_noise_std = noise_std * (1 + sensor_fault)  
        else:
            current_noise_std = noise_std
            
        noise = np.random.normal(loc=0, scale=current_noise_std, size=len(values))
